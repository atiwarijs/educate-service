...
import com.eduservices.profile.dto.PasswordChangeRequest;
import com.eduservices.profile.dto.PersonalDetailsDto;
import com.eduservices.profile.keycloak.KeycloakClient;
import com.eduservices.profile.keycloak.KeycloakRoleDto;
import com.eduservices.profile.keycloak.KeycloakTokenResponse;
import com.eduservices.profile.keycloak.KeycloakUserDto;
import com.eduservices.profile.keycloak.TokenRequestDto;
import com.eduservices.profile.keycloak.UserRegistrationDto;
import com.eduservices.security.config.KeycloakProperties;
import com.eduservices.security.entity.User;
import com.eduservices.security.entity.UserRole;
import com.eduservices.security.event.UserEventPublisher;
import com.eduservices.security.exception.AuthException;
import com.eduservices.security.exception.EmailAlreadyExistsException;
import com.eduservices.security.exception.KeycloakIntegrationException;
import com.eduservices.security.exception.ResourceNotFoundException;
import com.eduservices.security.repository.UserRepository;
...
//				userEventPublisher.publishUserCreated(UserMapper.toDto(savedUser));
	...
	public Map<String, Object> authenticate(String username, String password) throws AuthException {
		...
		try {
			...
			// Call keycloakClient with individual parameters instead of TokenRequestDto
			...
			KeycloakTokenResponse tokenResponse = keycloakClient.getToken(keycloakProperties.getRealm(), tokenRequest);
			...
			if (tokenResponse != null && tokenResponse.getAccessToken() != null) {
				...
				if (userId != null) {
					KeycloakUserDto userDto = keycloakClient.getUserById(keycloakProperties.getRealm(), userId,
							"Bearer " + adminToken);
			...
			throw new AuthException("Authentication service unavailable", e);
		}
	}
	...
	public String getUserIdByUsernameOrEmail(String usernameOrEmail, String accessToken) {
		...
		try {
			...
			if (isEmail) {
				// Search by email
				users = keycloakClient.getUsersByEmail(keycloakProperties.getRealm(), usernameOrEmail,
						"Bearer " + accessToken);
			} else {
				// Search by username
				users = keycloakClient.getUsersByUsername(keycloakProperties.getRealm(), usernameOrEmail,
						"Bearer " + accessToken);
			}
			...
			logger.error("Error looking up user ID by username/email", e);
			throw new KeycloakIntegrationException("Keycloak user lookup service unavailable", e);
		}
	}
	...
	public Map<String, Object> refreshToken(String refreshToken) throws AuthException {
		...
		try {
			...
			throw new AuthException("Token refresh service unavailable", e);
		}
	}
	...
	public String getAdminAccessToken() {
		try {
			...
			KeycloakTokenResponse tokenResponse = keycloakClient.getToken(keycloakProperties.getAdminRealm(),
					tokenRequest);
			...
			logger.error("Error retrieving admin token", e);
			throw new KeycloakIntegrationException("Keycloak admin service unavailable", e);
		}
	}
	...
	public String getUserIdByUsername(String username, String accessToken) {
		...
		try {
			...
			List<KeycloakUserDto> users = keycloakClient.getUsersByUsername(keycloakProperties.getRealm(), username,
					"Bearer " + accessToken);
			...
			logger.error("Error looking up user ID by username", e);
			throw new KeycloakIntegrationException("Keycloak user lookup service unavailable", e);
		}
	}
	...
	public KeycloakUserDto getUserInfo(String userId, String accessToken) {
		...
		try {
			...
			return keycloakClient.getUserById(keycloakProperties.getRealm(), userId, "Bearer " + accessToken);
			...
			throw new KeycloakIntegrationException("Keycloak user info service unavailable", e);
		}
	}
...
//				userEventPublisher.publishUserUpdated(UserMapper.toDto(user));
			...
			if (keycloakUpdateNeeded) {
				...
				keycloakClient.updateUser(keycloakProperties.getRealm(), userId, updatedUser, "Bearer " + accessToken);
				return true;
			} else {
			...
			throw new KeycloakIntegrationException("User update service unavailable", e);
		}
	}
	...
	public User registerUser(PersonalDetailsDto request, Principal principal) {
		...
		try {
			...
			KeycloakUserDto newUser = new KeycloakUserDto();
			...
			try {
				List<KeycloakUserDto> existingUsers = keycloakClient.getUsersByUsername(keycloakProperties.getRealm(),
			...
			try {
				...
				keycloakClient.createUser(keycloakProperties.getRealm(), newUser, "Bearer " + accessToken);
				...
				keycloakClient.resetPassword(keycloakProperties.getRealm(), keycloakUserId, credentialMap,
						"Bearer " + accessToken);
				...
				logger.error("Failed to register user in Keycloak: {} - {}", e.status(), e.getMessage());
	...
	private void assignKeycloakRoles(String userId, Set<String> roleNames, String accessToken) {
		...
		if (!rolesToAssign.isEmpty()) {
			try {
				keycloakClient.assignRealmRoles(keycloakProperties.getRealm(), userId, rolesToAssign, "Bearer " + accessToken);
	...
	private Optional<KeycloakRoleDto> getKeycloakRole(String roleName, String accessToken) {
		try {
			KeycloakRoleDto role = keycloakClient.getRealmRole(keycloakProperties.getRealm(), roleName,
					"Bearer " + accessToken);
			if (role != null) {
	...
	private KeycloakRoleDto createKeycloakRole(String roleName, String accessToken) {
		...
		keycloakClient.createRealmRole(keycloakProperties.getRealm(), newRole, "Bearer " + accessToken);
	...
	private void cleanupKeycloak(String keycloakUserId, String accessToken) {
		if (keycloakUserId != null) {
			try {
				...
				keycloakClient.deleteUser(keycloakProperties.getRealm(), keycloakUserId, "Bearer " + accessToken);
	...
	public ResponseEntity<?> updatePassword(String updaterEmail, String username, PasswordChangeRequest passRequest) {
		...
		try {
			...
			keycloakClient.resetPassword(keycloakProperties.getRealm(), userId, passwordPayload,
					"Bearer " + accessToken);

			user.setLastModifiedBy(updaterEmail);
			...
			throw new KeycloakIntegrationException("Password update service unavailable", e);
		}
	}
}